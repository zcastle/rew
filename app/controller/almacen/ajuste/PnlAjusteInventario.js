Ext.define('rewsoft.controller.almacen.ajuste.PnlAjusteInventario', {
    ...
    'almacen.ajuste.WinCantidad'
    ],
    refs: [{
        ref: 'MainView',
        selector: 'pnlajusteinventario'
    },{
        ref: 'WinAlmacenAjusteCantidad',
        selector: 'winalmacenajustecantidad'
    }],
    stores: [
    'Productos',
    'Lotes',
    ...
    init: function() {
        this.control({
            'pnlajusteinventario': {
                render: this.onRenderPnlAjusteInventario
            },
            'pnlajusteinventario grid[name=gridPedido]': {
                itemdblclick: this.onItemDblClickGridPedido,
                cellclick: this.onCellClickGridPedido
            },
            'pnlajusteinventario grid[name=gridProductos]': {
                render: this.onRenderedGridProductos,
                itemdblclick: this.onItemDblClickGridProductos
            },
            'pnlajusteinventario button[name=btnProcesar]': {
                click: this.onClickBtnProcesar
            },
            'pnlajusteinventario textfield[name=txtBuscar]': {
                keyup: this.onKeyUpTxtBuscar,
                keypress: this.onKeyPressTxtBuscar
            },
            'winalmacenajustecantidad': {
                afterrender: this.onAfterRenderWinCantidad
            },
            'winalmacenajustecantidad grid[name=gridLotes]': {
                itemclick: this.onItemClickGridLotes
            },
            'winalmacenajustecantidad button[name=btnAceptar]': {
                click: this.onClickBtnAceptar
            }
        });
    },
    onRenderPnlAjusteInventario: function() {
        this.getSecuencia();
    ...
    onClickBtnAceptar: function(button){
        var form = this.getWinAlmacenAjusteCantidad().down('form');
        if(form.getForm().isValid()){
            ...
            var co_almacen = button.up('window').down('displayfield[name=co_almacen]').getValue();
            var no_almacen = button.up('window').down('displayfield[name=no_almacen]').getValue();
            var gridPedido = this.getMainView().down('grid[name=gridPedido]');
    ...
    onClickBtnProcesarAjuste: function(grid, button){
        ...
        Ext.Msg.confirm('Confirmacion', 'Esta seguro de querer procesar la Guia de Ajuste No.: '+numeroDocumento+' ?', function(btn){
            if(btn=='yes'){
